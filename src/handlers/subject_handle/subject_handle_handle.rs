...
use crate::{
    controllers::subject_controller::subject_controller_controller::{
        create_subject, delete_subject_by_id, get_all_subject, get_subject_by_code,
        get_subject_by_id, get_subjects_by_class_room, update_subject_by_id,
    },
...
pub async fn get_subjects_by_class_room_handle(
    state: Data<AppState>,
    id: Path<String>,
) -> impl Responder {
    match change_string_into_object_id(id.into_inner()) {
        Err(e) => HttpResponse::BadRequest().json(e),
        Ok(i) => match get_subjects_by_class_room(state.into_inner(), i).await {
            Err(e) => HttpResponse::BadRequest().json(ReqErrModel {
                message: e.to_string(),
            }),
            Ok(r) => HttpResponse::Ok().json(r),
...
pub async fn get_subject_by_code_handle(
    state: Data<AppState>,
    code: Path<String>,
) -> impl Responder {
    match get_subject_by_code(state.into_inner(), &code.into_inner()).await {
        Err(e) => HttpResponse::BadRequest().json(ReqErrModel {
            message: e.to_string(),
        }),
        Ok(r) => HttpResponse::Ok().json(r),
